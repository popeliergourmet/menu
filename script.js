/* =============================================================
   POPELLIER · Pipoca Gourmet — script.js
...
(() => {
  ...
  const PRODUCTS = [
    ...
    {
      id: 'avela',
      name: 'Popellier Avelã',
      calda: 'Avelã',
      caldaKey: 'avela',
      desc: 'Pipoca gourmet dourada envolvida em creme de avelã.',
      image: 'images/product-avela-pour.png',
      badge: 'Indulgente'
    },
    {
      id: 'mix',
  ...
  const caldaLabel = (caldas) => caldas.length ? caldas.join(' + ') : '—';

  /* ---------- Persistência ---------- */
  const STORAGE_KEY = 'popellier.cart.v1';
  const loadCart = () => {
    try {
  ...
  function readBuilderState() {
    ...
    let caldas = $$('input[name="b-calda"]:checked').map(el => el.value);
    if (caldas.length === 0) {
      const first = $('input[name="b-calda"][value="Ninho"]');
      if (first) first.checked = true;
      caldas = ['Ninho'];
    }
    state.builder.caldas = caldas;
  ...
  function renderBuilder() {
    readBuilderState();
    const { size, caldas, toppings, qty } = state.builder;
    const { base, add, unit } = calcUnit(size, toppings);
    const total = unit * qty;

    $('#s-size').textContent = SIZES[size].label;
    $('#s-calda').textContent = caldaLabel(caldas);
  ...
  function bindBuilder() {
    ...
    // "Nenhum" ↔ toppings mutual exclusion (builder)
    section.addEventListener('change', (e) => {
      const t = e.target;
      if (t.name === 'b-none' && t.checked) {
        $$('input[name="b-topping"]').forEach(el => (el.checked = false));
      } else if (t.name === 'b-topping' && t.checked) {
        const none = $('input[name="b-none"]');
        if (none) none.checked = false;
      } else if (t.name === 'b-topping' && !t.checked) {
        const anyChecked = $$('input[name="b-topping"]:checked').length > 0;
        const none = $('input[name="b-none"]');
        if (!anyChecked && none) none.checked = true;
      } else if (t.name === 'b-none' && !t.checked) {
        const anyChecked = $$('input[name="b-topping"]:checked').length > 0;
        if (!anyChecked) t.checked = true;
      }
      renderBuilder();
    });
    ...
    $('#btn-add-cart').addEventListener('click', () => {
      ...
      const item = buildCartItem({
        name: 'Popellier ' + caldaLabel(state.builder.caldas),
        size: state.builder.size,
        caldas: state.builder.caldas,
  ...
  function updateWhatsBuilderLink() {
    ...
    if (!link) return;
    const { size, caldas, toppings, qty, obs } = state.builder;
    const { base, add, unit } = calcUnit(size, toppings);
    const total = unit * qty;
    const caldaTxt = caldaLabel(caldas);

    const lines = [
      'Olá, Popellier! Vim pelo site e gostaria de fazer um pedido:',
      '',
      `• ${qty}x Popellier ${caldaTxt}`,
  ...
  function openProductDrawer(productId) {
    ...
    if (!p) return;
    const initialCaldas = p.caldaKey === 'mix' ? ['Ninho', 'Avelã'] : [p.calda];
    state.pd = {
      productId: p.id,
      size: '500',
      caldas: initialCaldas.slice(),
      toppings: [],
      qty: 1,
      obs: ''
    };
    ...
    $$('input[name="pd-calda"]').forEach(el => (el.checked = initialCaldas.includes(el.value)));
    $$('input[name="pd-topping"]').forEach(el => (el.checked = false));
    const noneEl = $('input[name="pd-none"]');
  ...
  function readPdState() {
    ...
    let caldas = $$('input[name="pd-calda"]:checked').map(el => el.value);
    if (caldas.length === 0) {
      const first = $('input[name="pd-calda"][value="Ninho"]');
      if (first) first.checked = true;
      caldas = ['Ninho'];
    }
    state.pd.caldas = caldas;
  ...
  function bindPd() {
    ...
    drawer.addEventListener('change', (e) => {
      const t = e.target;
      if (t.name === 'pd-none' && t.checked) {
        $$('input[name="pd-topping"]').forEach(el => (el.checked = false));
      } else if (t.name === 'pd-topping' && t.checked) {
        const none = $('input[name="pd-none"]');
        if (none) none.checked = false;
      } else if (t.name === 'pd-topping' && !t.checked) {
        const anyChecked = $$('input[name="pd-topping"]:checked').length > 0;
        const none = $('input[name="pd-none"]');
        if (!anyChecked && none) none.checked = true;
      } else if (t.name === 'pd-none' && !t.checked) {
        const anyChecked = $$('input[name="pd-topping"]:checked').length > 0;
        if (!anyChecked) t.checked = true;
      }
      renderPdSummary();
    ...
    $('#pd-add').addEventListener('click', () => {
      ...
      const name = p ? p.name : ('Popellier ' + caldaLabel(state.pd.caldas));
      addToCart(buildCartItem({
        name,
        size: state.pd.size,
        caldas: state.pd.caldas,
  ...
  /* ---------- Cart ---------- */
  function buildCartItem({ name, size, caldas, toppings, qty, obs }) {
    const { base, add, unit } = calcUnit(size, toppings);
    const caldaArr = Array.isArray(caldas) ? caldas.slice() : [caldas].filter(Boolean);
    return {
      id: uid(),
      name,
      size,
      ...
      caldas: caldaArr,
      caldaLabel: caldaLabel(caldaArr),
      ...
      list.innerHTML = state.cart.map(it => `
        <li class="cart-item" data-id="${it.id}">
          <div>
            ...
            <p class="ci-meta">
              ${it.sizeLabel} · Calda ${escapeHtml(it.caldaLabel || (it.caldas ? it.caldas.join(' + ') : ''))} ·
  ...
  function updateCheckoutLink() {
    ...
    const lines = ['Olá, Popellier! Vim pelo site e gostaria de fazer o pedido:', ''];
    let subtotal = 0;
    state.cart.forEach((it, idx) => {
      subtotal += it.total;
      const caldaTxt = it.caldaLabel || (Array.isArray(it.caldas) ? it.caldas.join(' + ') : (it.calda || ''));
      lines.push(`${idx + 1}) ${it.qty}x ${it.name}`);
      lines.push(`   Tamanho: ${it.sizeLabel}`);
      lines.push(`   Calda: ${caldaTxt}`);
